...
import { PackageReceiveIcon, Plus, ArrowRight01Icon } from "@hugeicons/core-free-icons";
	...
	return (
		<div className="p-6 space-y-8">
			<div>
				...
				<p className="text-muted-foreground">Welcome to your product operating system.</p>
			</div>
						...
						<div className="space-y-4">
							{products.slice(0, 5).map((product) => (
								<div key={product.id} className="flex items-center justify-between border-b pb-2 last:border-0 last:pb-0">
									<div>
										...
										<p className="text-xs text-muted-foreground">{product.base_unit} • {new Date(product.created_at).toLocaleDateString()}</p>
									</div>
									<Button variant="ghost" size="icon">
										<Link to="/dashboard/catalog/products/$productId" params={{ productId: product.id }}>
							...
							{products.length > 0 && (
								<Button variant="link" className="px-0">
									<Link to="/dashboard/catalog/products">View all products</Link>
								</Button>
							)}
						</div>
					</CardContent>
